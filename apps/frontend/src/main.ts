...
import { mountBotDirectory } from './views/bot-directory.js';
  ...
  <div id="shell" style="display:flex;flex-direction:column;height:100vh;font-family:system-ui,sans-serif;">
    <nav id="nav" style="display:flex;align-items:center;justify-content:space-between;
      ...
      <div style="display:flex;align-items:center;gap:16px;">
        ...
        <button id="tab-directory" style="all:unset;cursor:pointer;padding:4px 12px;border-radius:4px;font-size:0.85rem;">Bot Directory</button>
...
function setActiveTab(tab: View): void {
  const tabs: Record<View, HTMLButtonElement> = { arena: tabArena, tournament: tabTournament, builder: tabBuilder, directory: tabDirectory, connect: tabConnect, howit: tabHowIt };
...
// ---- View management ----
type View = 'arena' | 'tournament' | 'builder' | 'directory' | 'connect' | 'howit';
...
async function switchView(target: View): Promise<void> {
  ...
  if (target === 'arena') {
    ...
    viewEl.appendChild(container);
    viewEl.style.overflowY = 'auto';
    await mountBotDirectory(container);
...
tabDirectory.addEventListener('click', () => void switchView('directory'));
